...
<section role="main" class="content-body">
 ...
 <?php echo form_open_multipart('Welcome/pramote');?>  
 		<div class="row">
			<div class="col-sm-2">
				<div class="mb-3">
...
<!-- 			</div> -->
			<div class="col-sm-6" style="float: left">	
				<div class="mb-6" >
					...
					<input class="btn btn-primary" id="clear" type="submit"  name="Clear All" value="Reset">
					<input class="btn btn-primary" id="Save" type="submit"  name="Save" value="Save All">
					<input class="btn btn-primary" id="selectAll" type="button"  name="select" value="Select All">
					 
				</div>
			</div>
		</div>    
 <?php if( isset($pramote_show)){  ?> 
				...
				<table class="table table-bordered table-striped mb-0" id="datatable-tabletools">
					<thead>
 					...
 					<tbody>    
					<?php  foreach($pramote_show as $attendance){ ?>
 					<tr >  
						<th><?php echo $attendance->roll_number;?></th>   
						<th><?php echo ucfirst($attendance->student_name);?></th>   
						<th>  
							<input  class="form-control myclass"   type="checkbox" name="status[]" <?php //if(isset($attendance->attendance) && $attendance->attendance == 'present'){ echo 'checked' ;} ?> value="<?php echo $attendance->id;?>" >
						</th>    
     				</tr> 	    
					<?php }?>  
     				</tbody>	
...
$(document).ready(function () {
	  $('#selectAll').on('click',   function () {
	    if ($(this).hasClass('allChecked')) {
	        $('input[type="checkbox"]', '#datatable-tabletools').prop('checked', false);
	    } else {
	        $('input[type="checkbox"]', '#datatable-tabletools').prop('checked', true);
	    }
...
var d = document.getElementById("pramote");
