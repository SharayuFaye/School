...
class m_marks extends CI_Model { 
    function marks_add($teacher_id,$student_id,$date,$exam_type,$marks,$out_of,$subject,$competence,$percentage,$school){ 
        $this->db->select("*");
        $this->db->from('marks');
        ...
        if($query->num_rows() == 0){
    		...
    		$this->db->insert('marks', $target);
    		return true;
        }
    }
    function marks_edit($id,$teacher_id,$student_id,$date,$exam_type,$marks,$out_of,$subject,$competence,$percentage,$school){ 
        $this->db->select("*");
        $this->db->from('marks');
        $this->db->where(array( 'students_id' => $student_id));
        $this->db->where(array( 'subject' => $subject));
    ...
    function marks_show(){  
		$this->db->select('s.student_name,s.roll_number,f.*,t.teacher_name,ex.type');
